...
const connectMetaMaskbsc = document.querySelector('.connectMetaMaskbsc');
...
// we need to check if it's there before adding the event listener to it
 if (connectMetaMaskbsc) {
  connectMetaMaskbsc.addEventListener('click', () => {
...
const connectButtonsbsc = [connectMetaMaskbsc/*, connectMetaMaskNav*/];
const displayConnectedButton = async () => {
  const accounts = await ethereum.request({ method: 'eth_accounts' });
  connectButtonsbsc.forEach((button) => {
